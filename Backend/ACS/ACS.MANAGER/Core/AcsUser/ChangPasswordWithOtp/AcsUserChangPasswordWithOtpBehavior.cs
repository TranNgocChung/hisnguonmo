        ...
        bool IAcsUserChangPasswordWithOtp.Run()
        {
            ...
            try
            {
                if (Valid())
                {
                    ...
                    if (AcsUserCheckVerifyValidDataForAuthorize.Verify(commonParam, ref user, optRawData.LOGINAME))
                    {
                        if (user != null && user.ID > 0)// && user.MOBILE == entity.Mobile
                        {
                            string encryptPassword = new MOS.EncryptPassword.Cryptor().EncryptPassword(entity.NewPassword, user.LOGINNAME);
